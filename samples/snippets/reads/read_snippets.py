...
# read_row('local-disk-361806','bigtable-ins-01','my-table')
...
def read_row_partial(project_id, instance_id, table_id):
    ...
    col_filter = row_filters.ColumnQualifierRegexFilter(b"os_name")

    row = table.read_row(row_key, filter_=col_filter)
    print_row(row)

# read_row_partial('local-disk-361806','bigtable-ins-01','my-table')
...
def read_rows(project_id, instance_id, table_id):
    ...
    row_set.add_row_key(b"r1")
    row_set.add_row_key(b"r2")

    rows = table.read_rows(row_set=row_set)
    for row in rows:
        print_row(row)

# read_rows('local-disk-361806','bigtable-ins-01','my-table')
...
def read_row_ranges(project_id, instance_id, table_id):
    ...
    row_set.add_row_range_from_keys(
        # start_key=b"phone#4c410523#20190501", end_key=b"phone#4c410523#201906201"
        start_key=b"r1", end_key=b"r2" 
    )
    row_set.add_row_range_from_keys(
        # start_key=b"phone#5c10102#20190501", end_key=b"phone#5c10102#201906201"
        start_key=b"r1", end_key=b"tablet#a0b81f74#2019"
    )
...
# read_row_ranges('local-disk-361806','bigtable-ins-01','my-table')
