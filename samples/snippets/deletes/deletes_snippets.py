...
def delete_from_column(project_id, instance_id, table_id):
    ...
    row = table.row("phone#4c410523#20190501")
    row.delete_cell(column_family_id="cf1", column="connected_wifi")
    row.commit()

delete_from_column('local-disk-361806','bigtable-ins-01','my-table')
...
def delete_from_row(project_id, instance_id, table_id):
    ...
    instance = client.instance(instance_id)
    table = instance.table(table_id)
    row = table.row("Raghav-r1")
    row.delete()
    row.commit()

# delete_from_row("local-disk-361806","bigtable-ins-01","my-table")
